...
- Dark instrument-panel header, light grey gradient body. Every section is collapsible — tap
  its header to expand/collapse (the down-triangle flips to show state). Active alerts starts
  open since it's the safety-critical one; everything else starts collapsed to keep the page
  scannable. Change the `open` class on any `<section>` in `index.html` to alter the defaults.
- A rotating sponsor bar at the bottom, with a "Sponsor via PayPal" slot built in (needs a
...
| Winter outlook | [The Old Farmer's Almanac](https://www.almanac.com/winter-extended-forecast-farmers-almanac) and [Farmers' Almanac](https://www.farmersalmanac.com/farmers-almanac-2026-2027-winter-weather-forecast) (two separate publishers) | Static text, updated once a year — see below |
...
### About the winter outlook section

Neither Almanac publisher offers an API, and their forecasts only change once a year, so this
is a hand-written summary (in the site's own words, not copied text) rather than a live feed.
It's labeled with the date it was last written and the winter it covers. To update it for a
future winter: search for that year's forecast from each publisher, rewrite the two blurbs in
your own words in the `data-key="almanac"` section of `index.html`, and update the "Last
updated" date next to it.

### Section header colors

Each section header has a light pastel tint, purely for visual scanability — no functional
meaning, just a way to tell sections apart at a glance. Defined near the top of the `<style>`
block in `index.html` as a set of `section[data-key="..."] .section-toggle { background: ... }`
rules; change any of them to retheme a section.

## Setting up the sponsor bar (open-source rotator + PayPal)

The bar at the bottom of the page rotates between sponsor slots. It's powered by
[AdShuffle](https://github.com/codions/ad-shuffle) (MIT license) — a small, genuinely
open-source rotation library, not a Google AdSense replacement disguised as one. It's
vendored into this repo at `vendor/ad-shuffle.js` rather than loaded from the author's own
domain, so the site doesn't depend on a low-traffic third-party CDN staying up; the only
change from the original is that the library's own auto-injected default styling (which
referenced an image on the author's domain) was stripped out in favor of this project's own
`.sponsor-slot` CSS.

**How rotation works:** each sponsor is a `<div class="rb-random-ads">` block inside the
`.rb-ads` container near the bottom of `index.html`. With `data-sequential="true"` (the
current setting) they cycle in order every `data-interval` seconds. Switch to
...
- **Colors, fonts, layout** are all in the `<style>` block at the top of `index.html`. The
  header stays dark (`--header-bg`) as a brand anchor; the body uses a light grey gradient
  (`--bg-1` → `--bg-2`); cards are white (`--surface`) with a light-grey-raised variant
  (`--surface-raised`) for nested content like gauges and data rows.
- **Search radii** (30 mi for river gauges, 75 mi for wildfires, 50 mi for shelters) are set
  in the `milesBBox(...)` calls inside `loadRivers`, `loadFires`, and `loadShelters`.
